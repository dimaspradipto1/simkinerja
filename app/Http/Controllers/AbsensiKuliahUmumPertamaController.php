...
use App\DataTables\AbsensiKuliahUmumPertamaDataTable;
...
class AbsensiKuliahUmumPertamaController extends Controller
{
    private function checkAdmin()
    {
        $authUser = auth()->user();
        if (!$authUser || (!$authUser->isAdmin() && !$authUser->isSuperAdmin())) {
            abort(403, 'Unauthorized action. Only Admin or Super Admin can manage Kuliah Umum attendance.');
        }
    }
     ...
     */
    public function index(AbsensiKuliahUmumPertamaDataTable $dataTable)
    {
        return $dataTable->render('pages.absensi-kuliah-umum-pertama.index');
    }
    ...
    public function create()
    {
        ...
        return view('pages.absensi-kuliah-umum-pertama.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('bukti_izin')) {
            $validated['bukti_izin'] = $request->file('bukti_izin')->store('absensi_kuliah_umum', 'public');
        }

        AbsensiKuliahUmumPertama::create($validated);

        Alert::success('Berhasil', 'Absensi Kuliah Umum Hari Pertama berhasil ditambahkan.');
        return redirect()->route('absensi-kuliah-umum-pertama.index');
    }
    ...
    {
        $absensi = AbsensiKuliahUmumPertama::with('user')->findOrFail($id);
        return view('pages.absensi-kuliah-umum-pertama.show', compact('absensi'));
    }
    ...
    {
        $this->checkAdmin();
        $absensi = AbsensiKuliahUmumPertama::findOrFail($id);
        $users = User::orderBy('name', 'asc')->get();
        return view('pages.absensi-kuliah-umum-pertama.edit', compact('absensi', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Alert::success('Berhasil', 'Data absensi Kuliah Umum Hari Pertama berhasil diperbarui.');
        return redirect()->route('absensi-kuliah-umum-pertama.index');
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Absensi Kuliah Umum Hari Pertama berhasil dihapus.'
        ]);
    }
    ...
    {
        $this->checkAdmin();
        $items = AbsensiKuliahUmumPertama::with('user')->orderBy('id', 'asc')->get();

        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Absensi Kuliah Umum Pertama');

        // Row 1: Title
        $titleText = 'LAPORAN ABSENSI KULIAH UMUM HARI PERTAMA';
        ...
        $filename = 'Absensi_Kuliah_Umum_Hari_Pertama_' . date('YmdHis') . '.xlsx';
    ...
    public function scanProses(Request $request)
    {
        ...
        $currentTime = now()->format('H:i');
        $absensi = AbsensiKuliahUmumPertama::where('user_id', $authUser->id)->first();

        if (!$absensi) {
            $absensi = new AbsensiKuliahUmumPertama();
    ...
    {
        return view('pages.absensi-kuliah-umum-pertama.scan');
    }
}
