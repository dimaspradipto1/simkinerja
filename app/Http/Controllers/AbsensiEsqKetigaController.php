...
use App\DataTables\AbsensiEsqKetigaDataTable;
...
class AbsensiEsqKetigaController extends Controller
{
     ...
     */
    public function index(AbsensiEsqKetigaDataTable $dataTable)
    {
        return $dataTable->render('pages.absensi-esq-ketiga.index');
    }
    ...
    public function create()
    {
        ...
        return view('pages.absensi-esq-ketiga.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        AbsensiEsqKetiga::create($validated);

        Alert::success('Berhasil', 'Absensi ESQ Hari Ketiga berhasil ditambahkan.');
        return redirect()->route('absensi-esq-ketiga.index');
    }
    ...
    {
        $absensi = AbsensiEsqKetiga::with('user')->findOrFail($id);
        return view('pages.absensi-esq-ketiga.show', compact('absensi'));
    }
    ...
    {
        $this->checkAdmin();
        $absensi = AbsensiEsqKetiga::findOrFail($id);
        $users = User::orderBy('name', 'asc')->get();
        return view('pages.absensi-esq-ketiga.edit', compact('absensi', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Alert::success('Berhasil', 'Data absensi ESQ Hari Ketiga berhasil diperbarui.');
        return redirect()->route('absensi-esq-ketiga.index');
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Absensi ESQ Hari Ketiga berhasil dihapus.'
        ]);
    }
    ...
    {
        $this->checkAdmin();
        $items = AbsensiEsqKetiga::with('user')->orderBy('id', 'asc')->get();

        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Absensi ESQ Ketiga');

        // Row 1: Title
        $titleText = 'LAPORAN ABSENSI ESQ HARI KETIGA';
        ...
        $filename = 'Absensi_ESQ_Hari_Ketiga_' . date('YmdHis') . '.xlsx';
    ...
    public function scanProses(Request $request)
    {
        ...
        $currentTime = now()->format('H:i');
        $absensi = AbsensiEsqKetiga::where('user_id', $authUser->id)->first();

        if (!$absensi) {
            $absensi = new AbsensiEsqKetiga();
    ...
    {
        return view('pages.absensi-esq-ketiga.scan');
    }
}
