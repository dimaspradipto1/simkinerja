...
use App\DataTables\AbsensiMiladPertamaDataTable;
...
class AbsensiMiladPertamaController extends Controller
{
     ...
     */
    public function index(AbsensiMiladPertamaDataTable $dataTable)
    {
        return $dataTable->render('pages.absensi-milad-pertama.index');
    }
    ...
    public function create()
    {
        ...
        return view('pages.absensi-milad-pertama.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        AbsensiMiladPertama::create($validated);

        Alert::success('Berhasil', 'Absensi MILAD Hari Pertama berhasil ditambahkan.');
        return redirect()->route('absensi-milad-pertama.index');
    }
    ...
    {
        $this->checkAdmin();
        $absensi = AbsensiMiladPertama::findOrFail($id);
        $users = User::orderBy('name', 'asc')->get();
        return view('pages.absensi-milad-pertama.edit', compact('absensi', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Alert::success('Berhasil', 'Data absensi MILAD Hari Pertama berhasil diperbarui.');
        return redirect()->route('absensi-milad-pertama.index');
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Absensi MILAD Hari Pertama berhasil dihapus.'
        ]);
    }
    ...
    {
        $this->checkAdmin();
        $items = AbsensiMiladPertama::with('user')->orderBy('id', 'asc')->get();

        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Absensi MILAD Pertama');

        // Row 1: Title
        $titleText = 'LAPORAN ABSENSI MILAD HARI PERTAMA';
        ...
        $filename = 'Absensi_MILAD_Hari_Pertama_' . date('YmdHis') . '.xlsx';
    ...
    public function scanProses(Request $request)
    {
        ...
        $currentTime = now()->format('H:i');
        $absensi = AbsensiMiladPertama::where('user_id', $authUser->id)->first();

        if (!$absensi) {
            $absensi = new AbsensiMiladPertama();
    ...
    {
        return view('pages.absensi-milad-pertama.scan');
    }
}
