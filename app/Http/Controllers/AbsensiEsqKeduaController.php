...
use App\DataTables\AbsensiEsqKeduaDataTable;
...
class AbsensiEsqKeduaController extends Controller
{
     ...
     */
    public function index(AbsensiEsqKeduaDataTable $dataTable)
    {
        return $dataTable->render('pages.absensi-esq-kedua.index');
    }
    ...
    public function create()
    {
        ...
        return view('pages.absensi-esq-kedua.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        AbsensiEsqKedua::create($validated);

        Alert::success('Berhasil', 'Absensi ESQ Hari Kedua berhasil ditambahkan.');
        return redirect()->route('absensi-esq-kedua.index');
    }
    ...
    {
        $authUser = auth()->user();
        $absensi = AbsensiEsqKedua::with('user')->findOrFail($id);
        if ($authUser && !$authUser->isAdmin() && !$authUser->isSuperAdmin() && $absensi->user_id !== $authUser->id) {
            abort(403, 'Unauthorized action.');
        }
        return view('pages.absensi-esq-kedua.show', compact('absensi'));
    }
    ...
    {
        $this->checkAdmin();
        $absensi = AbsensiEsqKedua::findOrFail($id);
        $users = User::orderBy('name', 'asc')->get();
        return view('pages.absensi-esq-kedua.edit', compact('absensi', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Alert::success('Berhasil', 'Data absensi ESQ Hari Kedua berhasil diperbarui.');
        return redirect()->route('absensi-esq-kedua.index');
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Absensi ESQ Hari Kedua berhasil dihapus.'
        ]);
    }
    ...
    {
        $this->checkAdmin();
        $items = AbsensiEsqKedua::with('user')->orderBy('id', 'asc')->get();

        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Absensi ESQ Kedua');

        // Row 1: Title
        $titleText = 'LAPORAN ABSENSI ESQ HARI KEDUA';
        ...
        $filename = 'Absensi_ESQ_Hari_Kedua_' . date('YmdHis') . '.xlsx';
    ...
    public function scanProses(Request $request)
    {
        ...
        $currentTime = now()->format('H:i');
        $absensi = AbsensiEsqKedua::where('user_id', $authUser->id)->first();

        if (!$absensi) {
            $absensi = new AbsensiEsqKedua();
    ...
    {
        return view('pages.absensi-esq-kedua.scan');
    }
}
