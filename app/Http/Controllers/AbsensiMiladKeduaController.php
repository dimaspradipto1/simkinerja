...
use App\DataTables\AbsensiMiladKeduaDataTable;
...
class AbsensiMiladKeduaController extends Controller
{
    private function checkAdmin()
    {
        $authUser = auth()->user();
        if (!$authUser || (!$authUser->isAdmin() && !$authUser->isSuperAdmin())) {
            abort(403, 'Unauthorized action. Only Admin or Super Admin can manage MILAD attendance.');
        }
    }
     ...
     */
    public function index(AbsensiMiladKeduaDataTable $dataTable)
    {
        return $dataTable->render('pages.absensi-milad-kedua.index');
    }
    ...
    public function create()
    {
        ...
        return view('pages.absensi-milad-kedua.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('bukti_izin')) {
            $validated['bukti_izin'] = $request->file('bukti_izin')->store('absensi_milad', 'public');
        }

        AbsensiMiladKedua::create($validated);

        Alert::success('Berhasil', 'Absensi MILAD Hari Kedua berhasil ditambahkan.');
        return redirect()->route('absensi-milad-kedua.index');
    }
    ...
    {
        $authUser = auth()->user();
        $absensi = AbsensiMiladKedua::with('user')->findOrFail($id);
        if ($authUser && !$authUser->isAdmin() && !$authUser->isSuperAdmin() && $absensi->user_id !== $authUser->id) {
            abort(403, 'Unauthorized action.');
        }
        return view('pages.absensi-milad-kedua.show', compact('absensi'));
    }
    ...
    {
        $this->checkAdmin();
        $absensi = AbsensiMiladKedua::findOrFail($id);
        $users = User::orderBy('name', 'asc')->get();
        return view('pages.absensi-milad-kedua.edit', compact('absensi', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Alert::success('Berhasil', 'Data absensi MILAD Hari Kedua berhasil diperbarui.');
        return redirect()->route('absensi-milad-kedua.index');
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Absensi MILAD Hari Kedua berhasil dihapus.'
        ]);
    }
    ...
    {
        $this->checkAdmin();
        $items = AbsensiMiladKedua::with('user')->orderBy('id', 'asc')->get();

        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Absensi MILAD Kedua');

        // Row 1: Title
        $titleText = 'LAPORAN ABSENSI MILAD HARI KEDUA';
        ...
        $filename = 'Absensi_MILAD_Hari_Kedua_' . date('YmdHis') . '.xlsx';
    ...
    public function scanProses(Request $request)
    {
        ...
        $currentTime = now()->format('H:i');
        $absensi = AbsensiMiladKedua::where('user_id', $authUser->id)->first();

        if (!$absensi) {
            $absensi = new AbsensiMiladKedua();
    ...
    {
        return view('pages.absensi-milad-kedua.scan');
    }
}
