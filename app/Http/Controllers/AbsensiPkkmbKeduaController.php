...
use App\Models\User;
use App\DataTables\AbsensiPkkmbKeduaDataTable;
...
class AbsensiPkkmbKeduaController extends Controller
{
    /**
     ...
     */
    public function index(AbsensiPkkmbKeduaDataTable $dataTable)
    {
        return $dataTable->render('pages.absensi-pkkmb-kedua.index');
    }
    ...
    public function create()
    {
        ...
        return view('pages.absensi-pkkmb-kedua.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        AbsensiPkkmbKedua::create($validated);

        Alert::success('Berhasil', 'Absensi PKKMB Hari Kedua berhasil ditambahkan.');
        return redirect()->route('absensi-pkkmb-kedua.index');
    }
    ...
    {
        $absensi = AbsensiPkkmbKedua::with('user')->findOrFail($id);
        return view('pages.absensi-pkkmb-kedua.show', compact('absensi'));
    }
    ...
    {
        $this->checkAdmin();
        $absensi = AbsensiPkkmbKedua::findOrFail($id);
        $users = User::orderBy('name', 'asc')->get();
        return view('pages.absensi-pkkmb-kedua.edit', compact('absensi', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Alert::success('Berhasil', 'Absensi PKKMB Hari Kedua berhasil diperbarui.');
        return redirect()->route('absensi-pkkmb-kedua.index');
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Absensi PKKMB Hari Kedua berhasil dihapus.'
        ]);
    }
    ...
    public function scanProses(Request $request)
    {
        ...
        $currentTime = now()->format('H:i');
        $absensi = AbsensiPkkmbKedua::where('user_id', $authUser->id)->first();

        if (!$absensi) {
            $absensi = new AbsensiPkkmbKedua();
    ...
    {
        return view('pages.absensi-pkkmb-kedua.scan');
    }
    ...
    {
        $this->checkAdmin();
        $data = AbsensiPkkmbKedua::with('user')->orderBy('id', 'asc')->get();
        ...
        $sheet->setCellValue('A1', "LAPORAN ABSENSI PKKMB HARI KEDUA");
        ...
        foreach ($data as $row) {
            ...
            // Apply light borders and alignments
            $bodyStyle = [
                'borders' => [
                    'allBorders' => [
                        'borderStyle' => Border::BORDER_THIN,
        ...
        $filename = 'Absensi_PKKMB_Hari_Kedua_' . date('YmdHis') . '.xlsx';
