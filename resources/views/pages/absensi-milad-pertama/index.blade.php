...
@section('content')
<div class="pagetitle">
    <h1>Data Absensi MILAD Hari Pertama</h1>
    <nav>
        <ol class="breadcrumb">
        ...
        <div class="col-lg-12">
            ...
            <div class="card border-0 mb-4 text-white shadow-sm absensi-mobile-banner" style="background: linear-gradient(135deg, #198754 0%, #a3cfbb 100%); border-radius: 0.75rem;">
                <div class="card-body p-4 d-flex flex-column gap-3 justify-content-center">
                    ...
                    <div>
                        <a href="{{ route('absensi-milad-pertama.scan') }}?start=1" class="btn btn-light rounded-pill px-4 py-2 font-semibold shadow-sm d-inline-flex align-items-center gap-2" style="color: #198754; font-weight: 600;">
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <!-- Export Excel Button -->
                        <a href="{{ route('absensi-milad-pertama.export-excel') }}" class="btn btn-success rounded btn-sm d-inline-flex align-items-center gap-1 font-medium bg-emerald-600 hover:bg-emerald-700 border-0">
                            <i class="bi bi-file-earmark-excel"></i> Export Excel
                        </a>
                        <!-- Add Button -->
                        <a href="{{ route('absensi-milad-pertama.create') }}" class="btn btn-primary rounded btn-sm d-inline-flex align-items-center gap-1 font-medium">
    ...
    <script>
        $(document).ready(function() {
            ...
            setInterval(function() {
                if ($.fn.DataTable.isDataTable('#absensimiladpertama-table')) {
                    $('#absensimiladpertama-table').DataTable().ajax.reload(null, false);
                }
            }, 5000);
        });
        ...
        function deleteAbsensi(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('absensi-milad-pertama') }}/" + id,
                        type: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                        success: function(response) {
                            if (response.success) {
                                ...
                                $('#absensimiladpertama-table').DataTable().ajax.reload();
                            } else {
                                Swal.fire({
                                    title: 'Gagal!',
